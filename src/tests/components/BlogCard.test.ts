...
describe('BlogCard', () => {
  ...
  it('renders blog entry information correctly', () => {
    ...
    expect(wrapper.find('a[href="/read/test-blog-post"]').exists()).toBe(true);
  });

  it('generates correct /watch/ URL for video entries', () => {
    const videoEntry = { ...mockBlogEntry, video: true };
    wrapper = mount(BlogCard, {
      props: { entry: videoEntry },
    });

    expect(wrapper.find('a[href="/watch/test-blog-post"]').exists()).toBe(true);
  });
  ...
  it('calls clipboard writeText when copy button is clicked', async () => {
    ...
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith(
      'https://localhost:3000/read/test-blog-post'
    );
  });
  ...
  it('has proper accessibility attributes', () => {
    ...
    const readMoreLink = wrapper.find('a[href="/read/test-blog-post"]');
    expect(readMoreLink.exists()).toBe(true);
    // Some links might not have aria-label, so just check they exist
    expect(readMoreLink.attributes('href')).toBe('/read/test-blog-post');
  ...
  it('handles responsive design classes', () => {
    ...
    const titleLink = wrapper.find('a[href="/read/test-blog-post"]');
  ...
  it('renders with correct link structure', () => {
    ...
    expect(readMoreButton.attributes('href')).toBe('/read/test-blog-post');
  });
